...
export default function CommentTextAria({comments, submitComment }) {

  const [newCommentBody, setNewCommentBody] = useState("");
  const [newScore, setNewScore] = useState(1);
  const[isAnawer,setIsAnswer]=useState(false)
    ...
    return (
      ...
      <div class="comments__respond">
        ...
        <div class="comments__respond-content">
          ...
          <textarea class="comments__score-input-respond" onChange={onChangeHandler}>
          {newCommentBody}

          </textarea>
        </div>
        <button type="submit" class="comments__respond-btn" onClick={() => submitComment(newCommentBody,newScore,isAnawer)} >
          ارسال
        </button>
      </div>
          </>
